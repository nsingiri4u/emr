...
export class Tabs extends Component {
	...
	handleMouseMove(event) {
		...
		if(this.dragTarget) {
			...
			if(index !== dragTarget.index) { //NS changed from != to !==
	...
	handleClose(index, event) {
		event.preventDefault();
		event.stopPropagation();
		this.hideTooltip(); //NS inserted this line
	...
	hideTooltip() {
		...
		//console.log('inside hide tooltip',this.refs.tooltip.style.display); NS inserted this line
	...
	render() {
		...
		let tabs = props.children.map((tab, index) => {
			...
			if(this.dragTarget && this.dragTarget.index === position) { //NS changed to === from ==
			...
			return (
				<div
					key={ index }
					className={ "tab-button" + (props.active === index ? " active" : "") } //NS changed to === from ==
					style={ style }
		...
		let panels = props.children.map((panel, index) => {
			if(props.active !== index) { //NS changed to !== from !=
				return null;
			}
...
const TabIcon = ({ type }) => {
	return (
		<div className={ "icon " + type }>
			{ type === 'loading' ? (<div className="mask"></div>) : null } {/*NS changed to === from ==*/}
		</div>
	);
};
